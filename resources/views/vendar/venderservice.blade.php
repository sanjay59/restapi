      ...
      <div class="col-md-12">
        <div class="portlet light" id="document-section">
          ...
          <div class="portlet-title">
            <div class="caption">
              <span class="caption-subject">Product Services</span>
            </div>
            <div class="actions">
              <button type="button" class="btn btn-primary" data-toggle="modal" data-target="#add-category-modal">Add Service</button>
            </div>
          </div>
          ...
          <div class="portlet-body">
            ...
            <div class="table-responsive">
              ...
              <tbody>
                @forelse($vcategorylist as $service)
                <tr>
                  <td><div class="numbring">{{$loop->iteration}}</div></td>
                  <td>{{$service->name}}</td>
                 
                  <td class="text-center">
                    <a href="javascript:void(0);" class="text-success mr-2" data-toggle="modal" data-target="#edit-category-modal" data-id="{{$service->id}}" >
                      <i class="fa fa-pencil"></i>
                    </a>
                       <a href="{{url('delete-vendor-service',$service->id)}}" class="text-danger" data-toggle="modal" data-target="#delete-category-modal"  data-id="{{$service->id}}">
  ...
  <div class="modal-dialog modal-dialog-centered">
    <div class="modal-content p-0">
      <div class="modal-header">
        <h5 class="modal-title mb-0">Add Product Service</h5>
      ...
      <div class="modal-body">
        ...
        <div class="success-msg text-center py-2 display-hide" id="add-thankyou-msg">
          ...
          <h3 class="mb-0">Product Service Add Successfully</h3>
        </div>
      </div>
    </div>
  </div>
...
<div class="comman-modal modal fade" id="edit-category-modal" tabindex="-1" role="dialog" aria-hidden="true">
  <div class="modal-dialog modal-dialog-centered">
    <div class="modal-content p-0">
      <div class="modal-header">
        <h5 class="modal-title mb-0">Edit Product Service</h5>
        <button type="button" class="close position-relative" data-dismiss="modal" aria-label="Close">
          <span aria-hidden="true">&times;</span>
        </button>
      </div>
      <div class="modal-body">
        ...
        <div class="success-msg text-center py-2 display-hide" id="edit-thankyou-msg">
          ...
          <h3 class="mb-0">Product Service Edit Successfully</h3>
        </div>
      </div>
    </div>
  </div>
    ...
    <div class="modal-content p-0">
      <div class="modal-body">
        ...
        <div class="success-msg text-center py-3 display-hide" id="delete-thankyou-msg">
          ...
          <h3 class="mb-0">Product Service Delete Successfully</h3>
        </div>
      </div>
    </div>
  </div>
...
<script type="text/javascript">
  $('#delete-category-modal').on('show.bs.modal', function (event) {
    ...
    $("#delete-btn").attr("href", "{{url('delete-vendor-service')}}/"+id);
  ...
  $('#edit-category-modal').on('show.bs.modal', function (event) {
    ...
    let _url = `{{url('edit-vendor-service')}}/${id}`;
    // console.log(_url);
    $.ajax({
      url: _url,
      type: "GET",
...
<script type="text/javascript">
  ...
  $('#categoryeditform').on('submit', function (e) {
   ...
   if(name == '')  
   {  
    $('.editname-error').html('Service is required'); 
  }  
  else{
    $('.editname-error').html(''); 
  }
  ...
  $('#categoryform').on('submit', function (e) {
   ...
   if(name == '')  
   {  
    $('.name-error').html('Service is required'); 
  }  
  else{
    $('.name-error').html(''); 
  }
  ...
  $('.sidebar-menu ul li.productivity a.product-service').css({'color':'#000'});
