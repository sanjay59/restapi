...
<div class="page-content-wrapper">
          ...
          <div class="portlet-body">
            <div class="table-responsive">
              <table class="table table-striped table-bordered table-hover">
                ...
                <tbody>
                  @forelse($getproduct as $d)
                  <tr>
                    <td><div class="numbring">{{$loop->iteration}}</div></td>
                    <td>
                      <div class="preview-thumb sm">
                        <a href="javascript:void(0);" data-toggle="modal" data-target="#document-modal" data-imgsrc="{{$d->p_file}}">
                          <img src="{{url('/')}}/assets/product/{{$d->p_file}}" class="w-100 h-100"></a>
                        </div>
                      </td>
                      <td>{{$d->evtname}}</td>
                      <td>{{$d->psname}}</td>
  ...
  <script type="text/javascript">
    $('#document-modal').on('show.bs.modal', function (event) {
      ...
      var modal = $(this)
      $("#showimg").attr("src", "{{url('/')}}/assets/product/"+imgsrc);
    });
  </script>
  <script type="text/javascript">
 ...
 <script type="text/javascript">
  ...
  $('.sidebar-menu ul li.productivity a.product-review').css({'color':'#000'});
